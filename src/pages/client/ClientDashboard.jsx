...
export function ClientDashboard({ user }) {
  ...
  async function handleCancel(orderId) {
    if (!window.confirm('Voulez-vous vraiment annuler cette réservation ? Cliquez sur OK pour confirmer l’annulation.')) return
    setOrderError('')
    try { await cancelOrder(orderId, user.id); setOrders(current => current.map(order => order.id === orderId ? { ...order, status: 'annulée' } : order)) } catch (exception) { setOrderError(exception.message) }
  }
  async function handleCancelAppointment(appointmentId) {
    if (!window.confirm('Voulez-vous vraiment annuler ce rendez-vous ? Cliquez sur OK pour confirmer l’annulation.')) return
  ...
  return <main className="container page"><div className="section-header"><div><span className="eyebrow">{t('nav.client')}</span><h1 className="section-title">{t('nav.client')}</h1></div><NavLink to="/client/dossiers/nouveau" className="btn btn-primary">{t('dossier.title')}</NavLink></div><div className="dashboard-grid">
    <section className="panel"><h2>{t(`${client}.my_files`)}</h2>{loading ? <div className="loading-state"><p>{t(`${client}.loading`)}</p></div> : error ? <div className="error-state" role="alert"><p>{error}</p><button type="button" className="btn btn-primary" onClick={() => window.location.reload()}>{t(`${client}.retry`)}</button></div> : dossiers.length === 0 ? <EmptyState title={t(`${client}.no_file`)} description={t(`${client}.no_file_description`)} action={<NavLink to="/client/dossiers/nouveau" className="btn btn-primary">{t(`${client}.first_file`)}</NavLink>} /> : <div className="list">{dossiers.map(dossier => <article key={dossier.id} className="list-card"><div><strong>{dossier.serviceId ? localizeService({ id: dossier.serviceId, name: dossier.service }, i18n.resolvedLanguage || i18n.language).name : dossier.service}</strong><div className="meta-line"><span>{dossier.id}</span><span>{dossier.date}</span></div></div><div><StatusBadge status={dossier.status} /><div className="card-actions"><NavLink to={`/client/dossiers/${dossier.id}`} className="btn btn-ghost">{t(`${client}.details`)}</NavLink><NavLink to={`/client/dossiers/${dossier.id}?chat=1`} className="btn btn-ghost">💬 {t('chat.contact_advisor')}</NavLink></div></div></article>)}</div>}</section>
